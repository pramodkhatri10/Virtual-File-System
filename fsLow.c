...
// This sets up a the file as a volume
// The volSize specifies how large the volume is in bytes
// blockSize specifies the size of each block (typically 512)
...
uint64_t LBAread (void * buffer, uint64_t lbaCount, uint64_t lbaPosition)
	...
	lseek (partInfop->fd, fl.l_start, SEEK_SET);
	uint64_t readVal = read(partInfop->fd, buffer, fl.l_len);

	fl.l_type = F_UNLCK;
	fcntl(partInfop->fd, F_SETLKW, &fl);
	
	return readVal/partInfop ->blocksize;
	}
